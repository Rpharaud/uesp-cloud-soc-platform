...
invalid_names=$(find . \
  ...
  | grep -E '[A-Z ]' \
  | grep -vE '^./(README|DESIGN|THEORY|TESTPLAN|RISKS|IMPLEMENTATION|EVIDENCE|DETECTIONS|CHANGELOG|SECURITY|OPERATIONS)\.md$|^./docs/architecture/[A-Z_]+\.md$|^./governance/.*/[A-Z_]+\.md$' \
  || true)
